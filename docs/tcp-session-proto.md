...
> 不改协议头结构**。
> v1.4（2026-09-06）：**下行停-等升级为滑动窗口**（见 §6.1）——转发器发送窗口 `DWIN`（=guest `TCP_RXWIN`）
> ×独立 seq、guest 接收侧重排缓冲 + 累计 ACK 推进 `rx_next`、转发器最老未确认槽超时重传；与上行
> 完全镜像，`MSG_ACK` 累计语义不变、**协议头结构仍不变**。§6.3 候选自此落地。
...
## 6. 可靠性语义（v1.4：下行 + 上行双向滑动窗口）

### 6.1 下行滑动窗口（v1.4 已落地，host→guest）

> 动机：v1.1 把 `TCP_RXB` 提到 4096 只"抬高丢尾阈值"，未消掉 NIC/socket 界面 burst 随机丢包的
> 根因（>16KB 依旧缺尾，见 bugs.md BUG-047 收尾）。v1.2 下行改**可靠停-等**从根子上消除缺尾；
> v1.4 把下行停-等升级为**滑动窗口**（与 §6.2 上行镜像），吞吐从"1/RTT"提至"W/RTT"。

- 每个 host→guest `MSG_DATA` 的 `flags` 低 16 位 = 递增**下行 seq**（从 0 起，每会话累计）。
- **转发器发送侧**（滑动窗口发送）：最多 `DWIN`（=8，guest `TCP_RXWIN`）个下行数据报同时在途
  （`dl_win` 保留副本）；收到 guest 累计 ACK（下一期望下行 seq）才推进 `dl_base` 腾窗续发；最老
  未确认槽每 `RETX_MS=2.0s` 超时重传（SR 风格）。`eof` 且数据发尽、窗口空后才发 `MSG_CLOSED`。
- **guest 接收侧**（滑动窗口接收）：seq 落在窗口 `[rx_next, rx_next+TCP_RXWIN)` 内即进重排缓冲
  `rx_win`（乱序先到缓存），`rx_flush` 凑齐连续后依序送 `rxb`、累计推进 `rx_next`，回累计 ACK
  （payload = `rx_next`）；窗口外/已交付区重复丢弃载荷、重发 ACK 自愈。重复 seq 同值覆盖、不重交付。
- **累计 ACK**：`MSG_ACK` payload = "下一期望下行 seq"（2B 大端）。所有 seq < value 均已确收，
  转发器据此一次性推进 `dl_base` 多个槽位。SLIP 慢通道单报回环 ~1s，重传间隔须 ≥ 此值
  （60ms 会灌爆慢 UART）；e1000 快通道回环 <10ms，正常无丢不触发重传，2s 只在真丢时恢复。
- 语义等价：下行变成**最多 8 报/窗的流水线可靠字节流**，老 tcp_recv 感知不到重传与乱序调度。
...
### 6.3 下行滑动窗口（v1.4 已落地，性能项兑现）

上行滑窗落地后，host→guest 下行本停-等（转发器 ≤1 报在途）。v1.4 已把下行升级为滑动窗口（§6.1）：
转发器发送侧 + guest 接收侧双双做窗口（各自镜像上行的收发逻辑），确认从"单报"提升为"累计"。
技术即上行滑窗的镜像，协议头结构不变（`MSG_ACK` 累计语义复用），已由宿主确定性测试
`tests/test_downlink_window.py`（窗口上限/累计推进/最老槽重传/不缺尾）与 `test_tcp_dl` 端到端覆盖。
...
5. 可靠下行（§6.1）：下行 DATA 带下行 seq，发送侧滑动窗口在途 ≤ DWIN，收到 guest 累计
   `MSG_ACK`（下行确认，下一期望 seq）才推进窗口续发，最老未确认槽超时 `RETX_MS=2.0s` 重传。
