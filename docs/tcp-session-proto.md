...
## 6. 可靠性语义（v1.2，上下行均可靠停-等；滑动窗口为候选）
...
### 6.2 上行可靠（已落地，guest→host）

> 对称地把停-等搬到上行：guest 是"发送方"，转发器是"接收方"。不同于下行（转发器主动拉、guest
> 被动收），上行由 **guest 负责重传**：guest 保留在途副本，超时未收确认即重发；转发器对已收到的
> 上行 seq 去重、按序转发到真 TCP，并回 host→guest `MSG_ACK`（上行确认）。

- guest `tcp_send(fd,d,n)`（v1.2 起为**阻塞停-等**，见 tcp-thin-api §1）：把 `d` 存入发送副本
  （`tx_inflight`），分配上行 seq（`tx_seq`，自 0 递增），发 `MSG_DATA`（带该 seq），然后阻塞等
  host→guest `MSG_ACK`（下一期望上行 seq == `tx_seq+1`）；超时按 `TCP_TX_TICKS=2.5s` **重发在途副本**
  （复用同一 seq，转发器幂等去重）。
- 转发器 `up_next` 计数器：收到上行 `MSG_DATA` 的 seq==`up_next` → 转发载荷到真 TCP、`up_next++`、
  回 `MSG_ACK(up_next)`；seq 重复/乱序（ACK 丢后 guest 重发）→ **丢弃载荷**、重发 `MSG_ACK(up_next)`
  （guest 据此确认"已在途推进"，自愈）。
- 语义等价：上行变成**可靠字节流**（guest 循环多次 `tcp_send` 即可可靠上传大文件；每条返回 =
  已被转发器确收）。这是"薄→厚"第一级台阶的补全——收发两方向 guest 都开始参与 seq/ACK，
  但仍**无窗口/流控，一条消息一个往返**（停-等），非完整状态机。
