...
**虚拟 TCP 薄→厚演进候选**：下行可靠停-等 + **上行可靠停-等**（v1.2 均落地）是"薄→厚"的第一级
台阶——guest 收发两方向都开始参与 seq/ACK（收顺序包回 ACK、发送保留副本带 seq 等 ACK），但**仍属
薄包装加固**：guest 无窗口/流量控制、一条消息一个往返（停-等），不是完整状态机。红线依旧：
guest **不做真 TCP 状态机**，演进上限 = 厚包装伪状态机。下列滑动窗口为下一步候选，按"每步可
独立验证、CI 全绿才进下一步"推进，不破坏薄包装 API、转发器会话表与协议头结构（GRANT 预留点）。
