...
### 支线 A：继续做深 x86 内核（v0.15+）
- ~~用户栈守卫页（guard page）与栈溢出检测~~ ✅ v0.13 已完成
- ~~更完整的文件系统（目录层级、文件偏移定位/追加写、间接块）~~ ✅ v0.14 已完成
- 网络：NIC 驱动 + 极简协议栈（建议的 v0.15）
- 真实硬件引导（GRUB/ISO）——串口终端（v0.10）已就绪，届时可直接在真机串口上交互调试
- 可选：v0.12 起演示进程的 fork/exec 尚无完整"父进程等待"语义（如 wait 竞态修复只是第一步），
  可补：wait 支持等待任意已退出子进程、exec 失败回滚后 shell 反馈、进程树/孤儿清理
...
> 建议顺序：先按支线 A 把 x86 内核做扎实（v0.14 文件系统增强已完成，建议继续 v0.15 网络或
> 补全进程 wait 语义），等 x86 特性攒够、且真有 ARM 目标时再启动 HAL 重构——因为独立地址空间
> 已牵动页表/切换，届时抽象 HAL 收益最大、返工最少。HAL 属破坏性重构，需开分支。
