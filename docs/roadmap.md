...
* **record/replay「尺子」元工程演进（自检与防假阴性，系统工程视角的最佳实践沉淀）**
  ...
  * **防假阴性四原则（对尺子的加固，按回报排序）**：
    ...
    * **② golden 自举 = 标定缺陷**：`.out.tr` 的"黄金"来自首次录制——若那次带错，回放永远
      "一致地错"。对策：**注入外部标定**（独立 checker 重算契约指纹、抽检人工核对 golden 边角），
      不把"首次录制"当作先验正确。
  ...
  * **交付动作（最小集，暂不立项，节能）**：① 在 README/复现文档里写死"先辨尺子再辨缺陷"的
    ...
    ③ 掩蔽清单以注释形式就地登记到 `rp_torture.sh` / 判据脚本。三者纯增量、不动录放主路径、
    不碰真 TCP/SMP/HAL——与本仓库"纯增量、可复现测试"红线一致。
...
**虚拟 TCP 薄→厚演进候选**：下行可靠停-等 + **上行可靠停-等**（v1.2）→ **上行滑动窗口**（v1.3）
落地，是"薄→厚"的技术台阶——guest 收发两方向都参与 seq/ACK（收顺序包回 ACK、发送侧有**窗口**
与超时重传，不再一次一包等 ACK），但**仍属薄包装加固**：guest 只做轻量发送缓冲与窗口管理，
不做完整 TCP 状态机 / 流量控制。红线依旧：guest **不做真 TCP 状态机**，演进上限 = 厚包装伪状态机。
后续候选按"每步可独立验证、CI 全绿才进下一步"推进，不破坏薄包装 API、转发器会话表与协议头结构。

* **✅ 上行可靠（guest→host，v1.2 已落地）**：把停-等**对称搬到上行**——guest `tcp_send` 保留
  `tx_inflight` 发送副本 + 分配上行 seq，发 `MSG_DATA` 后阻塞等 host→guest `MSG_ACK`（上行确认），
...
* **✅ 上行滑动窗口（v1.3 已落地）**：停-等"发 1 等 ACK"改 **N 在途**——guest `tcp_send` 把载荷写入
  **发送窗口**（`TCP_TXWIN=8` 个槽，各带独立 seq），窗位有空即发、**返回 n 即入窗已发出**（不再阻塞等
  ...
  `tx_base`，最老未确认槽每 `TCP_TX_TICKS` 超时重传。转发器为滑动窗口**接收**端：窗口内乱序包暂存
  `up_buf` 凑齐连续后按序投 `tcp.sendall`、回**累计 ACK**；重复 seq 覆盖不重投。大文件上传=循环
