...
### 支线 A：继续做深 x86 内核（v0.13+）
- 用户栈守卫页（guard page）与栈溢出检测——fork/exec 已能创建任意进程，可加保护
- 更完整的文件系统（目录层级、文件偏移定位/追加写、间接块）
- 网络：NIC 驱动 + 极简协议栈
...
- 风险提示：这是较大重构，建议先用一个独立分支做，避免破坏 x86 主线可运行状态

> 建议顺序：先按支线 A 完成 1~2 个版本把 x86 内核做扎实（含独立地址空间），
> 再启动 HAL 重构——因为独立地址空间会牵动页表/切换，此时抽象 HAL 收益最大、返工最少。
