...
| v0.29 | 加固：宿主 fuzz + 内核堆审计 | 宿主侧确定性 fuzz（`tests/fuzz_parse.c`，确定性 PRNG 注入畸形路径/字节，ASan 清洁 60000 轮/36 万次解析调用，覆盖 fs_walk/elf_load/net_* 解析恶意帧）；内核堆审计 `heap_audit()`（magic/free/size 上界 + used/free 记账对账，防泄漏/双free/破坏块头）并入 kern_audit；修 BUG-029（icmp 短帧越界读）、BUG-030（fork 子进程继承已生长栈误判缺页） |
...
> 版本 v1.1（2026-09-01），状态：**已对齐待开工**。做真 netif 抽象层，把协议层与具体
...
- [ ] **Step 4：宿主转发器 + 虚拟 TCP demo**：会话协议头（`session_id` + `msg_type` +
  ...
  背压；UDP 直转；事件回传 msg_type=event）；guest 虚拟 TCP 薄包装 API
  `tcp_open/send/recv/close`（fd = **连接对象**结构，非裸整数，厚包装时同一对象加状态机
  字段即可）。验收：HTTP 请求-响应 demo 跑通 + 断连/超时有明确失败返回值无挂死 + 事件回传可用

**虚拟 TCP 语义边界（诚实声明：这是包装，不是 TCP）**：薄包装只给「状态可见」面——
流式做不到（仍按报文）、可靠由宿主重传（guest 无感）、状态靠转发器显式回传的连接事件感知
（`msg_type=event` 决定薄包装体验下限）。GRANT：连接句柄=对象、收发走流式缓冲、控制/数据
通道分离、协议头版本字段先行四点现在就必须按"薄→厚演进"设计，将来厚包装应用零改动、帧格式
不破坏、转发器新旧会话共存。
