...
> `tx_inflight` 副本 / `tx_seq` / 阻塞等 ACK）实现落地，均为对象内状态、**不改 API 签名与返回
> 语义的互斥性**（send 由"一发即返回"升级为"确收后返回 `n`"，仍返回字节数；见 §1）。「滑动窗口」
> 候选仍只在对象内加状态、不破接口（见 roadmap"薄→厚演进候选"）。
...
| send | `int tcp_send(int fd, const uint8_t* d, uint32_t n)` | `n` | **v1.2 起为"阻塞停-等"可靠发送**：载荷保存到发送副本，带上行 seq 发出，阻塞等 host→guest `MSG_ACK`（上行确认）后返回 `n`；超时（重传上限）返回 -1。载荷 ≤ 单包上限（见 MTU 规约）。语义从"一次 send 一发即返回"升级为"一次 send 一发且确收后返回"——对调用方透明（返回 `n` 即确认递达转发器），大文件上传靠循环多次 `tcp_send` 即可 |
