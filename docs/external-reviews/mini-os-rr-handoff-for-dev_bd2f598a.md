...
| ---- | -------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **F1** | ✅ 已修复 | tests/replay.sh          | `replay_into` 由 `rel_ms delta sleep` 改「等上一行 ack 计数到位再注入下一行」的行级背压；ack 信号为消费路径 `[sched] wake keyboard waiter pid=.. (N bytes)` 与 `[kb] readline pid=.. -> N bytes` 的并集计数。`rel_ms` 保留仅作审计。                                                                               | 12 writefile+12 rm 多行流回放：普通时钟 **rm 12/12、无 unknown、残留 0**；`REPLAY_ICOUNT=1`（F1 原始失败条件）同样 **12/12、无 unknown、残留 0**。test-tr / test-rp / test-repro 全绿。                                    |
| **F2** | ✅ 已修复 | tests/transcript.sh      | `tr_send` 注入后等本行 readline 消费确认（同 ack 并集计数自增）再返回；超时 → `error: input ack timeout` **exit 2**（0/1/2 环境病语义对齐）。`tr_emit_in` 改在 ack 后记 `rel_ms`，使 in.tr 即"消费时间轴"（F4 附带）。调用方经 `TR_LOG` 注入串口日志路径（test_transcript/test_replay/repro_bugs/rp_torture 已接线）。 | 录制 golden 12 writefile+12 rm 全量落盘（rm 12/12、余 0），不复现审核侧"selftest 整条丢失"。                                                                                                                       |
| **F3** | ⏳ 未处理 | —                        | 拆 PR 后再补（test_tcp_attack.sh 端口占用 → exit 2）。                                                                                                                                                                                                                                                             | —                                                                                                                                                                                                             |
