...
## BUG-074 \[已修复] v1.5 堆空闲链表成环 → kfree 合并死循环整机冻结（#132 回归根因，PR #136）

* **版本**：v0.29（kfree 合并引入）潜伏；#132 回归（cc500 膨胀改变分配序列）暴露；v1.5 根治
* **危险度**：高 —— 链表成环后 kfree 的合并循环 `for(o=head; o; o=o->next)` 永不 NULL 死循环；
  且该路径位于 cli 段（exec 的 load_elf_file 内 kfree，定时器中断被屏蔽）→ **整机永久冻结**，
  串口心跳/键盘/网络全部停止，只能重启 QEMU。
* **触发条件（三级依赖）**：低 16MB 帧水位偏移 → exec cc500 的帧分配序列变化 → 堆空闲链表成环
  （`[heap] kfree: chain loop/suspect`，b=5b6000 系列）。0c3852b 曾用 `-fno-inline` 压 cc500
  产物规避触发（未治本）。
* **根因**：kfree 合并条件按"物理地址相邻"（`high == low+16+size`）扫描**全链表**找配对块；
  但空闲链表**不按地址排序**（`heap_add_pages` 新块插 head、`block_claim` 分裂插中间），
  物理相邻的两个块在链表中可能**顺序相反**（high 在 low 之前）。此时
  `low->next = high->next` 把 low 接回 high 之后（更早的链位）→ next 指针成环。
  且合并循环**无防环上限**（heap_audit 有 max_blocks，kfree 没有）→ 成环即死循环。
  QEMU 寄存器实锤：EIP 停在 kfree+0x8a，EFLAGS.IF=0（cli 段）。
* **修复（两轮）**：
  1. **行为修复（PR #136，先合）**：kfree 只与**链表相邻**的前驱/后继空闲块合并，
     物理相邻但链序不相邻的保留碎片（内核堆仅 ~20 块，碎片代价可忽略，正确性优先）；
     防环上限保留（`page_count*4096/24+4`，与 heap_audit 同口径）——任何异常不再升级为
     cli 段整机冻结，降级为可诊断日志。
  2. **结构根治（L3，随本 BUG 记录）**：空闲链表改为**按地址排序**插入
     （`heap_add_pages` 按地址找插入点；`block_claim` 分裂出的块天然紧跟在 b 之后，无需改）。
     「物理相邻 ⇔ 链表相邻」成为**结构不变量**——即便将来合并逻辑回退为"全链表扫描物理相邻"，
     物理相邻的块在链表中也必然相邻，环在结构上不可能形成。`heap_audit` 增加地址序断言
     （链表须按地址严格递增），违约立即报告。
* **回归**：宿主单测 test_heap 新增"物理相邻但链序相反"构造用例——断言 kfree 后无环、
  heap_audit 通过、地址序断言能检出被手动破坏的顺序；test_cc500 全绿（宿主 74/74 +
  guest 自举不动点）；test_socket 全绿（含 heap_audit）；宿主单测 20/20。
* **教训**：凡"按内容条件（物理地址）修改无顺序保证的链表"都是成环引信；不变量若只靠
  "这次写对"维持，迟早被下一个改动破坏——正确姿势是让**数据结构本身**（地址序）
  承载不变量，人无法违反它。

## BUG-075 \[已修复] kb 行缓冲"行就绪丢弃窗口"：多行粘贴/heredoc 只进第一行（readline 输入丢失）

* **版本**：v0.9（行缓冲引入）起存在；v0.30 防两行合并（BUG-034）改用 `!line_ready` 守卫后
  成为显式行为；本次根治。
* **危险度**：中 —— 交互/agent 经串口粘贴多行内容时，除第一行外**静默丢失**（无任何告警），
  只能分批粘贴（每次等 shell 取走再粘下一段）；heredoc 写多行源码同样受影响。
* **触发条件**：readline 消费慢于输入流时（QEMU stdio 整段粘贴、串口暴灌），
  `line_ready=1`（上一行已定行、尚未被 `kb_line_take` 取走）窗口内到达的**所有**可打印字符
  与换行符被 `!line_ready` 守卫丢弃。中断处理远比 shell 用户态取行快，故一次粘贴多行
  必然只进第一行。
* **根因**：`kb_feed_char` 两处 `if (!line_ready)` 守卫——`\n`/`\r` 只在"行未就绪"才定行、
  可打印字符只在"行未就绪"才入行缓冲。行就绪（待取）期间输入**无缓冲地**被忽略；
  单行缓冲模型下没有"等待窗口"外的容纳结构。
* **修复（挂起行队列，KB_LINE_Q=4）**：
  1. **定行即入队、组装行立即让位**：`\n`/`\r` 把整行复制入环形挂起队列并清空组装行
     （保留 v0.30 防两行合并的本质——新字符开始新行、绝不追加进旧行）；
  2. **队列满丢新行 + 告警**：超过 KB_LINE_Q 行未取走时丢弃新定行并打一条
     `[kb] warning: pending-line queue full`（不覆写队头槽，避免并发取行撕裂）；
  3. **多进程循环派发**：`sched_wake_keyboard` 改为循环——每轮唤醒一个 BLOCK_KEYBOARD
     等待者、取一行，直到无行或无可唤醒者（多进程同时 readline 各得一行，不再只服务第一个）。
* **回归**：宿主单测 test_kb 用例 12/13 按新语义更新（未取走时的再次回车 → 入队空行并
  再次触发回调；行未取期间的新字符 → 组新行、两次 take 各得 "a" 与 "bc"、绝无串行）；
  手动 QEMU 一次性注入 `hello\nls\nhelp\n` 三行全部执行（旧实现只进第一行）；
  test-host 20/20、test-cc500 全绿（guest heredoc 逐行拼接）、test-serial 全绿、
  make test 全链通过。
* **教训**：单行缓冲 + "就绪即拒收" 的守卫是"慢消费者丢输入"的引信——终端输入丢失对
  agent 是静默故障（整段粘贴只剩第一行、源码残缺编译报错却查不出输入问题）；
  正确姿势是给输入一个**有界队列**（背压），把"慢"从丢失降级为保留+显式告警。

## BUG-076 \[已修复] cc500 goto 未定义标签从不被拒（lbl_end 锚点读错位，#161）

* **版本**：M11（goto/labels）引入起存在。
* **危险度**：中 —— `goto` 到本函数内从未定义的标签，编译器**不报错**、照常产出 ELF 并运行
  （跳向未知地址 = 运行时未定义行为）；教学面"写了 goto 却忘写标签"被静默放行。
* **根因**：标签记录布局为「名字…NUL | 1B flag | 4B 目标」，锚点 = **名字 NUL 位置**；
  `lbl_find`/`lbl_declare`/`lbl_def_anchor`/`emit_goto_anchor` 均按此跳名取值，唯
  `lbl_end` 从记录**起点**把 `t+1` 当 flag 读——读到的是名字第二字符，`'u'` 判定**恒假**，
  终检从未生效。旧测试 `t_gundef` 的 rc=1 期望长期由 `done:;` 的空体分号走 expression
  错路"意外绿"，把该盲区掩盖了两个大版本周期（#160 合法化空语句后伪装脱落，CI 如实报案）。
* **修复**：`lbl_end` 先扫名字到 NUL（e），再按 flag(`e+1`)/target(`e+2`) 判定、下条记录 `e+6`。
* **回归**：`test_cc500` 加 T 系列三钉（`t_gundef` 期望回正 rc=1 —— 这次真由 lbl_end 拦、
  非体分号错路；`t_gosemi` 原文案形态同拒；`t_godef` 定义在后者不误伤）；
  mc_matrix G4 编译 rc 钉同步。
* **教训**："已绿的东西可能一直绿得错误"——本仓审计锁第三次实证：门禁的价值不只在防回归，
  也在于让**伪装脱落**。

## BUG-077 \[已修复] cc500 入口选错：main 之前的函数体夺走入口，main 成死代码（#162）

* **版本**：v0.27（入口 stub 编组 argv/argc 起，入口=首个函数体）起存在。
* **危险度**：高 —— 静默产出与使用者预期**完全不同**的程序：main 根本不执行，入口跑别的
  函数并以其返回值 exit。`int f(){return 1;}int main(){return f()-1;}` 一律 exit 1，
  极易误诊为"调用结果进算术就算错"（本仓 #162 立案时的原始误判即此）。
* **触发条件**：源里存在 main 之外的函数体，且写在 main 之前。
* **根因**：入口 stub 的 `call rel32` 只在**首个函数体**处回填一次——自举必须如此
  （cc500.c 首函数是 `cc500_main` 且全文件无 main）；源里 main 更靠后时，无人再改入口。
* **修复**：对名为 `main` 的函数体**再**回填一次入口 rel32（main 在首时与旧码同值）→
  入口=main，对齐 minicc/gcc 语义；无 main 时保持旧契约（首函数体=入口），自举不受影响
  （启动不动点 P1==P2 仍是闸门）。
* **回归**：票面三形态（`f()-1` / `f()+g()-3` / `x=f();return x-1`，f/g 定义在 main 前）
  现均 exit 0（gcc 同值）；mc_matrix 新增 G4 三运值钉 + cc500 编译 rc 钉。
  golden 侧：`g03_call`（全库唯一"函数体在 main 之前"的语料）随之从**静默错编译**变为
  可运行正确例——跑出 rc=0；其 cc500 产物哈希声明后重生成，minicc 及其余 6 例零漂移。
* **教训**：产物哈希基线只锁"字节一致"、不锁"语义正确"——`g03_call` 一直以错编译的产物
  入基线而无人察觉；凡"能编译但入口/语义可疑"的语料，必须另有**运行值钉**兜底。
