...
* **自举容量重标定，并把"容量↔内核体积"的耦合解开**：
  ...
  - 节点池：15 个并行数组由**静态数组改为运行时 brk 分配**。关键发现：minicc 不分离 `.bss`，
    ...
    自举产物 **497,496 B → 155,215 B（-69%）**，`NMAX` 可一次给到 49152（对全量 `IN_LIMIT` 留 23% 余量）。
  - 产物：`code_cap` 860000 → 500000（155 KB 产物的 3 倍余量）。
  - 定容依据**改用自举源自身的实测**：源 65052 B 时 16384 不足、20480 够 ⇒ 密度 ≈ 0.269~0.315 节点/字节。
    宿主 minicc 的密度只有一半左右——两份实现各建各的 AST，**拿宿主数据给自举源定容是本轮踩过的坑**，
    已在源码「自举容量」段写明。
