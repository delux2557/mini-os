...
- 自举过程的已知问题与对策：
  - 节点池越界写穿全局 `nn`（BUG-038）：并行数组有效下标 0..`NMAX-1`，`node_new` 守卫用 `nn >= NMAX - 1`，否则 `nnlocals[NMAX]` 会写穿到相邻全局把节点计数清零。（#172 后数组改为运行时分配，守卫语义不变。）
  ...
  - 资源上限（**#172 重标定 + 后续订正**，定容依据见 `minicc_self.c` 顶部「自举容量」段）：输入 `IN_CAP=131072`/上限 `IN_LIMIT=126976`（差一个读块余量）、节点池 `NMAX=49152`、名字池 `STRTAB_CAP=131072`、符号/补丁/标签表 `SYM_MAX=512`/`PATCH_MAX=4096`/`LAB_MAX=4096`、`code_cap=500000`。
    ...
    - **符号表 / 补丁表 / 标签表同样由静态数组改 brk**（照 #172 的做法；此前只有节点池与名字池改了，这三条被漏在外面）。改前它们是**贴边的容量悬崖**：静态数组按字节全量内联进产物（实测 29 B/符号位、12 B/补丁位、12 B/标签位，旧配置 256/2048/2048 合计 **56,576 B = 产物的 40%**），而自举源的真实需求已达 **238 符号 / 1,973 补丁 / ≈1,250 标签**，余量仅 18 / 75 / ~800 位——符号与补丁只剩个位数百分比（#182 goto 镜像一个特性就吃掉 6 个符号位）。改 brk 并抬到与 host 对齐（512/4096/4096）后：产物 **142,638 → 87,349 B（−55.3 KB）**，余量 2.1× / 2.0× / 3.3×。
      - 一处必需的非显然改动：**brk 内存不保证为零**，而 `finish()` 用 `lpos[lab] < 95` 识别"标签已分配但从未发射"，依赖的正是静态数组的隐式零初始化 ⇒ 三张表分配后必须显式清零（`zero_ints`/`zero_chrs`），否则读到垃圾 ≥95 会跳过守卫、把补丁写向野地址。
      - 一处已计入的代价：改 brk 后**每个表引用点多占 1 个补丁位**（指针全局需地址补丁，静态数组可直接寻址）⇒ 补丁需求 1,973 → 2,057（+84，+4%）。
    - 仍是静态数组的（未动，实测占位）：循环帧栈 16,384 B、`strpool` 4,096 B、goto 表 4,032 B、`tok` 256 B 等，合计 **≈25 KB** 仍计入产物。其中只有循环帧栈带容量含义，且受"循环嵌套 ≤32 / 每循环 ≤64 个 break|continue"守卫约束，不是可由源涨破的池式天花板。**旧文档此处写"`PATCH_MAX=4096`、`strpool[4096]` 维持静态（合计约 8KB，占比已小）"——该数字既把 host 的 `PATCH_MAX` 当成了 self 的，也完全漏掉了补丁/标签/符号三表的真实占位（56.6 KB），已作废。**
    - 天花板的门禁：FAST 层 `mc_matrix` 的 S1/S2a~S2e（可编译子集 / 读块余量 / 源 ≤75% IN_LIMIT / 实测产物 <90% code_cap / 节点池未回退 / 名字池 ≥ 源×0.5）+ CI 的 `miccboot` 层真编一次（**节点峰值、符号/补丁/标签峰值都只能实测**——两实现的规模不可互推，故这一层不可省）。
