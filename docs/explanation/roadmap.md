...
| 工程质量   | 分层测试、零告警、纯逻辑可单测、文档与代码同 PR 门禁 | 工程成熟度高    |
| 代码规模   | ~500KB 级 / 版本线已入 v1.x        | 接近维护临界点   |
...
- **✅ 原"残余待收口"已闭环（v1.5）**：e1000 DHCP BOOTP 组帧与续约状态机不再由内核
  直调/承载——`e1000_dhcp_tick` 已删除，续约状态机整体移至用户态 dhcpclient
  （syscall#40-44 原子能力，见 [design.md 12 章 DHCP 续约节](../explanation/design.md)）；
  该项不再推迟到 HAL 阶段。
### 红线（明确不做）
