...
- 纵深底线（BUG-055）：`pf_handler` 对**内核态命中用户半区未映射页**（CPL=0 且
  fault ∈ 用户半区）由 `cli;hlt` 整机停机**降级为 `sched_kill(current)` 杀进程**——即便某条
  新 syscall 漏掉了 `user_ptr_valid` 收敛，最坏也只是该进程被杀，整机存活；栈按需生长
  （CPL=3 分支先行）与懒分配（低地址区）不受影响，仅用户半区例外降级。
- ELF 加载区间钳制（BUG-056）：`load_elf_file` 强制 `p_memsz` 派生区间 ≤1MB 且整体落在用户
  半区，否则 -1——堵"小文件声称 96MB 段"的映射/帧耗尽向量；`elf_load` 在 `mapfn` 失败即中止
  （不再清 bss 未映射区）。注意 mm 层"页表帧落低 16MB 恒等区、否则按虚拟地址解引用即错"的
  隐含假设仍是残余 P2（元数据帧专用池待立）。
