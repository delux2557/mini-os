...
/* ---- 内存池 ----
 * V4 arena 瘦身：小枚举字段用 char（nkind/nty/nbty/nvkind 值恒 <256；其余节点句柄/偏移仍须 int）。
 * ⚠ **两个"值恒 <256"的例外**（同为把"计数/尺寸"当小枚举塞进 char 造成，2026-09-18）：
 *   - nnargs（实参/形参计数，本编译器允许到 int）：257 被截断成 1 ⇒ "1 形参函数 + 257 实参"
 *     的 arity 校验被静默绕过，调用点 `add esp` 只清 1 个实参 ⇒ 泄漏 256 个实参所占的栈；
 *     而实参 128~255 时 signed char 变负 ⇒ 反向误拒合法 C（安全复核 F6 家族）。现为 int。
 *   - nnlocals（局部帧字节数，上限 LOCAL_BYTES_MAX=4096）：按 256 取模截断（328→72、256→0）
 *     ⇒ 产物 prologue 的 `sub esp,imm` 随之写小、帧内局部数组越界写栈。此前自举源里没有帧
 *     ≥256 的函数，该假设一直没被打破；本分支给 stmt() 新增 `char bb[256]` 后帧首次越过 256，
 *     才经由 miccboot 的逐字节比对显形（编译期/功能面皆不报，同类"三面皆不报"）。现为 int。
