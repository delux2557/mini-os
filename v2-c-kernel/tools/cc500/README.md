...
| M4 | `+= -= *= %=` 复合赋值 | 算术复用 `binary2`（emit 与对应二元运算同字节）+ `'='` 的 lvalue 地址保持存回；token 从 `+-*%` 侧合成 `X=`（从 `'='` 侧判定会把 `x=-x` 吞成 `=-`）；**修复 `a += 1` 被静默编成 `a = 1`**（旧违纪律#2）；lvalue 单次求值（`a[f()] += x` 的 f 只调一次） | `feat(cc500)` M4 |
| 候选 | `&& \|\|` 短路、`++/--`、`/=` | 短路需表达式内跳转；`++/--` 需前/后置区分与 lvalue 保持，风险较高，独立评估；`/=` 与 M4 同机制可低成本追加 | — |
...
`+=` load 旧值编码锁定 + 非 lvalue/`/=` 报错断言）+ guest 层（ccboot 自举 P1==P2 +
ccrun 运行语义，含 for/do/一元/^~/乘除模/优先级/复合赋值/lvalue 单次求值用例）。
