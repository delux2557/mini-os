...
- **指针访问（M13，收 #165）**：`*p` 解引用（读 / 写）、`*(p+k)`、`p[i]` 下标、`&x` 取址均受支持，
  与 minicc（本轮补上 `p[i]` 脱糖 `*(p+i)`）在**语法面**对称。但 cc500 **无类型面** ⇒
  `*p` / `p[i]` 恒按 char 宽取值存值，`int *` 的语义与 minicc / gcc **有意不同**（两侧都编得过
  但值不同）；且不做指针性检查——`*3` 编得过（运行期自成后果），`&3` 编译期拒。
  跨编译器差分取样只取 char 宽形态。
- **编译错误诊断**：`error()` 打印 `cc500: error at <token>`（未闭合字符串→`bad string`、未定义符号→`undefined symbol`）。
- **字符串转义集合**（M9g，与 minicc `decode_escape` 逐项对齐）：`\n \t \0 \\ \" \'`，以及
  `\x` **贪心**吃后续全部十六进制位（上限 7 位、按 char `&0xFF` 截断）；**未知转义一律编译期拒**
  （`cc500: error`）。与 gcc 的有意差异：gcc 对 `\x` 无位数上限（超长仅截断）、未知转义只告警
  并按原字符处理——故同一源码两侧可能"一编一拒"，差分取样须避开或按此口径过滤（宁拒不坑）。
